...
def detect(im, param_vals):
    ...
    # Converting the image to various scales.
    scaled_ims = list(make_scaled_ims(im, model.WINDOW_SHAPE))

    # Loading the model which detects number plates over a sliding window.
    x, y, params = model.get_detect_model()

    # Executing the model at each scale.
    ...
    # Doing this by identifying windows (at all scales) where the model predicts a
