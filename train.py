...
def train(learn_rate, report_steps, batch_size, initial_weights=None):
    ...
    # y_ =tf.compat.v1.placeholder(shape=[None, 2], dtype=tf.float32)
    ...
    def do_report():
        r = sess.run([best,correct,tf.greater(y[:, 0], 0),y_[:, 0],digits_loss,presence_loss,loss],feed_dict={x: test_xs, y_: test_ys})
        num_correct = numpy.sum(numpy.logical_or(numpy.all(r[0] == r[1], axis=1),numpy.logical_and(r[2] < 0.5,r[3] < 0.5)))
        ...
        print ("B{:3d} {:2.02f}% {:02.02f}% loss: {} "
               "(digits: {}, presence: {}) |{}|").format(batch_idx,100. * num_correct / (len(r[0])),100. * num_p_correct / len(r[2]),r[6],r[4],r[5],"".join("X "[numpy.array_equal(b, c) or (not pb and not pc)]for b, c, pb, pc in zip(*r_short)))
...
if __name__ == "__main__":
    ...
    train(learn_rate=0.001,report_steps=20,batch_size=50,initial_weights=initial_weights)
